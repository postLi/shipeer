<template>
  <div class="height_100 flex_r">
    ...
    <div class="flex_1 b_c_w o_f" >
      ...
      <!--发货地 目的地 -->
      <el-form :model="form" ref="orderRules" class="myForm-m">
        <div class="item-margin item-4 flex_r" v-for="(item,index) in form.to">
          ...
          <div style="max-width: 940px;" class="width_100">
            <div class="flex p_r">
              <div class="item-4-1 flex_1 flex_sb p_r" :style="{'border-color':item.checkMap?'#dcdfe6':'red'}">
                <div class="flex_1 input flex_a" @click="showMap1(item,index)">
                  <img src="../../assets/main/tihuod.png" class="pointer" alt="">
                  <!--<span class="window-title-12 pointer" :class="[(item.origin === '')?'c-c':'c-0']">{{(item.origin === '')?'必填':item.origin}}</span>-->
              ...
              <el-button size="small" v-if="index === 1" type="success" icon="el-icon-circle-plus" style="width: 104px;background-color: white;color: #2fb301" @click="addNewDestination(form.to.length)">新增目的地</el-button>
              <el-button size="small" v-if="index > 1 " type="danger" icon="el-icon-remove" style="width: 104px;background-color: white;color: #ff300d" @click="delNewDestination(index)">删除目的地</el-button>
            </div>
            <div class="flex_sb item-base0">
              <div class="item-base margin_t_20 flex_a">
              ...
              <div class="item-base margin_t_20 flex_a" :style="{'border-color':item.checkP?'#dcdfe6':'red'}">
                ...
                <el-form-item label="" label-width="" :prop="'to.' + index + '.consigneeMobile'" size="small"
                              :rules="{required: true, validator:(rule, value, callback)=>{
                                  return checkMyPhone(rule, value, callback,item)
                                }, trigger: 'blur'}">
                  <el-input class="myInput-border-none" style="font-size: 12px" v-model="item.consigneeMobile" :placeholder="(index === 0)?'发货联系人电话（必填）':'收货联系人电话（选填）'"></el-input>
                </el-form-item>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...mapActions(['setForm']),
      ...mapGetters(['getForm']),
      checkMyMap(rule, value, callback,item){
        if (value === '') {
          callback(new Error("收发货地址必填"));
          item.checkMap = false
        }else {
          callback();
          item.checkMap = true
      ...
      checkMyPhone(rule, value, callback,item){
        if(item.shipperSort === 0){
          item.checkP = checkPhone(rule, value, callback,0)
        }else {
          item.checkP = checkPhone(rule, value, callback,1)
        }
      },
