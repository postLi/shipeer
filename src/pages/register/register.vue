      ...
      <div class="content flex">
        <div class="content-center" v-if="!nextPage">
            ...
            <el-form :model="form1" ref="Rules1" class="myForm">
              <div class="item flex_as">
                ...
                <el-form-item label="" label-width="" prop="imageCode"
                              :rules="{required: true,validator: checkImageCode2, trigger: 'blur'}">
                  <el-input class="myInput"
                            style="width: 228px"
                            autosize
              ...
              <div class="item flex_as">
                ...
                <el-button :disabled="canClick" class="code-img f_w"
                           style="background-color: #3b99f0;width: 133px;height: 40px;font-size: 16px;border-radius: 0"
                           type="primary" @click="getCode()">{{timeName === '获取验证码'?'获取验证码':'发送成功（' + timeName + '）'}}
                </el-button>
              </div>
            </el-form>
          </div>
          <div class="item flex_a">
            ...
            <div>
              ...
              <span class="pointer">
                  <span class="window-title-16 c-3" @click="checked = true">我已阅读并同意</span><span @click="user()"
                                                                                                class="window-title-16 c-f">《用户协议》</span>
                </span>
            </div>
          </div>
          <div class="item flex_a margin_t_20">
            <div class="item-left window-title-16 c-3"></div>
            <el-button class="f_w"
                       style="background-color: #2fb301;width: 183px;height: 50px;font-size: 20px;border-radius: 0"
                       type="success" @click="next()">下一步
            </el-button>
          </div>
        </div>
        <div class="content-center-success flex_f" v-else>
          ...
          <div class="flex margin_t_20">
            <el-button class="f_w" style="background-color: #2fb301;width: 128px;height: 44px;font-size: 18px;"
                       type="success" @click="login()">去登录
            </el-button>
          </div>
        </div>
      </div>
...
<script>
  ...
  import {loginCode, validLoginCode, checkImageCode} from '@/api/login'
    ...
    methods: {
      ...
      getCode() {
        this.$refs['Rules'].validate((valid) => {
          if (valid) {
            this.$refs["Rules1"].validateField("imageCode", errMsg => {
              if (errMsg)
                return;
              let parm = {
                memberType: 'AF0010101',
      ...
      checkImageCode2(rule, value, callback) {
